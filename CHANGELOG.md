...
### Added
- Add `InputFile` parameter to `New-AsBuiltReport`, allowing a report to be generated from one or more previously exported/offline system configuration files instead of connecting live to a target system. `Target` is optional in this mode and, if supplied, must map 1:1 with `InputFile`. Support for `InputFile` must be implemented individually by each report module.
